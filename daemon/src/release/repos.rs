...
const PROPRIETARY_SOURCES: &str = concatcp!(PPA_DIR, "nux-apps.sources");
const GROOVY_PPA: &str = concatcp!(PPA_DIR, "nux-ppa.list");
const PPA_SOURCES: &str = concatcp!(PPA_DIR, "nux-ppa.sources");
const IMPISH_RELEASE: &str = concatcp!(PPA_DIR, "nux-release.sources");
...
fn update_preferences_script(release: &str) -> anyhow::Result<()> {
    ...
    fs::write("/etc/apt/preferences.d/nux-default-settings", data.as_bytes())
        .context("failed to overwrite nux-default-settings apt preferences")
}
...
fn groovy_ppa(release: &str) -> String {
    format!(
        r#"## This file was generated by nux-upgrade
#
...
mod tests {
    ...
    fn is_save_file() {
        ...
        assert!(!super::is_save_file(Path::new("/etc/apt/sources.list.d/nux-apps.sources")));
        assert!(super::is_save_file(Path::new("/etc/apt/sources.list.d/nux-apps.sources.save")));
    }
}
